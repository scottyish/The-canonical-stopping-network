...
#     print(events_to_run:=events_1.reset_index().event_type.unique())
    ...
    for event_type in events_to_run:
        ...
        glm1.add_event(event_type, basis_set='canonical_hrf_with_time_derivative',  interval=[0, 20], show_warnings=False)
...
if is_interactive():
    ...
    import subprocess
    import itertools
    os.system('jupyter nbconvert --to script 07b_hierarchical_ROI_GLM_GdH.ipynb')  # make script from this notebook
    ...
    def call_shell(roi, model_n, t_r):
        subprocess.run(["ipython", "07b_hierarchical_ROI_GLM_GdH.py", roi, str(model_n), str(t_r)]) 
...
# this_ = fit_single_roi('IFG-r', pd.read_pickle(f'../derivatives/hierarchical_roi_glm/Leipzig_7T_GdH/timeseries/Leipzig_7T_GdH_timeseries.pkl'),
#                       pd.read_pickle(f'../derivatives/hierarchical_roi_glm/Leipzig_7T_GdH/events/Leipzig_7T_GdH_events.pkl'), '0', 'Leipzig_7T_GdH',
#                       2.0, True, None)
