...
def fit_glm(timeseries, events, confounds, include_rois, model_n=0, t_r=2.0, oversample_design_matrix=10, concatenate_runs=False, fit_type='ols'):
...
#     print(events_to_run:=events_1.reset_index().event_type.unique())
...
if is_interactive():
    ...
    import subprocess
    import itertools
    os.system('jupyter nbconvert --to script 07b_hierarchical_ROI_GLM_aron.ipynb')  # make script from this notebook
    ...
    def call_shell(roi, model_n, t_r):
        subprocess.run(["ipython", "07b_hierarchical_ROI_GLM_aron.py", roi, str(model_n), str(t_r)]) 
...
# mod = fit_single_roi('IFG-r', pd.read_pickle(f'../derivatives/hierarchical_roi_glm/aron_3T/timeseries/aron_3T_timeseries.pkl'),
#                       pd.read_pickle(f'../derivatives/hierarchical_roi_glm/aron_3T/events/aron_3T_events.pkl'), '0', 'aron_3T',
#                       2.0, True, None)
...
# list(mod._model.results.to_dataframe().columns)


# In[70]:


# new_sigs = signals[['IFG-r']]
# new_evs = events.loc[events.event_type=='go']


# In[85]:


# plt.plot(t, new_sigs.iloc[(new_sigs.index.get_level_values('subject')=='01') & (new_sigs.index.get_level_values('run')==1)])


# In[78]:


# t = list(new_sigs.index.get_level_values('time').unique())
...
# new_sigs.iloc[(new_sigs.index.get_level_values('subject')=='01') & (new_sigs.index.get_level_values('run')==1)]
...
# mod.get_group_timecourse_traces()


# In[55]:


# new_sig = signals['IFG-r']
...
# fac = mod.plot_subject_timecourses(col_wrap=3, legend=True)
...
# # plot ground truth
# t =np.linspace(0, 18, 182)
# for subject, ax in enumerate(fac.axes.ravel()):
#     mod.response_fitters[subject].plot_timecourses()
# #     subject += 1
# #     ax.plot(t, hrf*pars.loc[subject, 'fs'].amplitude, ls='--', lw=1.5, color=sns.color_palette()[0])
# #     ax.plot(t, hrf*pars.loc[subject, 'ss'].amplitude, ls='--', lw=1.5, color=sns.color_palette()[1])
# #     ax.plot(t, hrf*pars.loc[subject, 'go'].amplitude, ls='--', lw=1.5, color=sns.color_palette()[2])


# In[91]:


# mod.response_fitters[0].get_original_signal()


# In[92]:


# mod.response_fitters[0].get_standard_errors_timecourse()


# In[102]:


# mod.response_fitters[0].plot_timecourses()


# In[103]:


# mod.response_fitters
...
# events.loc[events['event_type'].isin(['fs', 'ss','go'])]
...
# def fit_single_roi(roi, df, events,  model_n, dataset, overwrite=False):
...
#     fit_single_roi(roi, df, events, model_n='0', dataset=dataset)
